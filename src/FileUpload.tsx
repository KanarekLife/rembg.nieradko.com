...
import { CheckCircle, Trash01, Loading01, UploadCloud02, XCircle, DownloadCloud02 } from "@untitledui/icons";
...
import type { RemoveBackgroundResult } from "rembg-webgpu";
import { Compare } from "@/components/base/compare/compare";
...
const getFileName = (fileName: string): string => {
    return `rembg-${fileName || 'image.png'}`;
}

const handleDownload = (result: RemoveBackgroundResult & { fileName: string }) => {
    if (!result?.blobUrl) return;

    const link = document.createElement('a');
    link.href = result.blobUrl;
    link.download = getFileName(result.fileName);
...
export const FileListItemProgressBar = ({ name, progress, error, onDelete, onRetry, className, result, previewUrl }: FileListItemProps) => {
    ...
    return (
        <motion.li
            layout="position"
            className={cx(
                "relative rounded-xl bg-primary p-4 ring-1 ring-secondary transition-shadow duration-100 ease-linear ring-inset",
        ...
        >
            <div>
                {previewUrl && (
                    <Compare firstImage={previewUrl} secondImage={result?.blobUrl} slideMode="drag" />)
                }
            </div>
            <div className="flex gap-3">
                <div className="flex min-w-0 flex-1 flex-col items-start">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-0.5 flex items-center gap-2">
                                <div className="flex items-center gap-1">
                                    ...
                                    {!isComplete && !failed && <Loading01 className="stroke-[2.5px size-4 text-fg-quaternary" />}
                                    {!isComplete && !failed && <p className="text-sm font-medium text-quaternary">Processing...</p>}

                                    {failed && <XCircle className="size-4 text-fg-error-primary" />}
                                    {failed && <p className="text-sm font-medium text-error-primary">Failed: {error}</p>}
                                </div>
                            </div>
                        </div>

                        <ButtonUtility color="tertiary" tooltip="Delete" icon={Trash01} size="xs" className="-mt-1 -mr-2 self-start" onClick={onDelete} />
                    </div>
                    ...
                    {isComplete && result && (
                        <Button color="primary" size="sm" onClick={() => handleDownload(result)} className="mt-1.5 w-full" iconLeading={DownloadCloud02}>
                            Download
                        </Button>
                    )}
                </div>
...
export const FileListItemProgressFill = ({ name, size, progress, error, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    const isComplete = progress === 100;
    const failed = !!error;
                        ...
                        <div className="mt-0.5 flex items-center gap-2">
                            ...
                            {!failed && (
                                <>
                                    ...
                                    <div className="flex items-center gap-1">
                                        {isComplete && <CheckCircle className="size-4 stroke-[2.5px] text-fg-success-primary" />}
                                        {!isComplete && <Loading01 className="size-4 stroke-[2.5px] text-fg-quaternary" />}
